...
async function connectDevice(usePairingCode = false) {
  ...
  async function attemptConnection() {
    try {
      if (getConnection() && (await isConnectionActive(getConnection()))) {
...
async function sendDocument(recipient, mediaPath, caption, fileName, msgid) {
  ...
  const connection = getConnection() || ((await connectWaBeforeSend()) && getConnection());
  if (!connection) return false;
  const mediaMessage = await prepareMediaMessage(connection, {
    caption: caption ?? '',
    fileName,
    media: mediaPath,
    mediatype: 'document',
  ...
  const userId = connection.user.id.replace(/:\d+/, '');
